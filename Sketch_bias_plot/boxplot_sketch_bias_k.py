...
def main():
    ...
    parser.add_argument('-k', '--thresholds', nargs='+', type=int, required=True,
                        help='Two threshold values for k (threshold1 threshold2)')
    parser.add_argument('--ylim', nargs='+', type=float, default=[0.05, 0.035, 0.05],
    ...
    # Ensure exactly two thresholds are provided
    if len(args.thresholds) != 2:
        print("Error: You must provide exactly two threshold values for k")
        return
...
# Function to create boxplots with three panels based on provided thresholds
def create_triple_subplot_boxplot(data, thresholds, selected_theta_values=[0.1, 0.01], 
                                 ylimits=[0.05, 0.035, 0.05], fixed_r=0.01):
    # Ensure we have two thresholds
    if len(thresholds) != 2:
        print("Error: Need exactly 2 threshold values")
        return
    ...
    if len(ylimits) < 3:
        ylimits = ylimits + [0.05] * (3 - len(ylimits))
    ...
    print(f"Unique k values in data: {k_values}")
    ...
    small_k_data = filtered_data[filtered_data['k_value'] <= threshold1]
    medium_k_data = filtered_data[(filtered_data['k_value'] > threshold1) & (filtered_data['k_value'] <= threshold2)]
    large_k_data = filtered_data[filtered_data['k_value'] > threshold2]
    ...
    print(f"Small k values (≤ {threshold1}): {small_k_values}")
    print(f"Medium k values ({threshold1} < k ≤ {threshold2}): {medium_k_values}")
    print(f"Large k values (> {threshold2}): {large_k_values}")
    
    # Create the melted DataFrame for boxplot
    # Important: Prepare the data in a format that boxplot can use
    plot_data = []
    
    for k in k_values:
        k_data = filtered_data[filtered_data['k_value'] == k]
        
        # Add r_strong values
        for _, row in k_data[k_data['theta'] == selected_theta_values[0]].iterrows():
            plot_data.append({
                'k_value': k,
                'value': row['r_strong'],  # Use 'value' instead of 'estimated_r'
                'method': 'r_strong'
            })
        
        # Add r_sketch values for each theta
        for theta in selected_theta_values:
            theta_data = k_data[k_data['theta'] == theta]
            for _, row in theta_data.iterrows():
                plot_data.append({
                    'k_value': k,
                    'value': row['r_sketch'],  # Use 'value' instead of 'estimated_r'
    ...
    def plot_on_axis(ax, data, k_values, title, ylim=None):
        ...
        # Create the boxplot - use 'value' column for y-axis
        sns.set(style="whitegrid", font_scale=1.2)
        sns_plot = sns.boxplot(x='k_value', y='value', hue='method', 
                         data=data,
        ...
        ax.set_ylabel('Estimated r Value', fontsize=18)
    ...
    # Filter plot data for each panel
    small_plot_data = plot_df[plot_df['k_value'] <= threshold1]
    medium_plot_data = plot_df[(plot_df['k_value'] > threshold1) & (plot_df['k_value'] <= threshold2)]
    large_plot_data = plot_df[plot_df['k_value'] > threshold2]
    ...
    # Save the plot
    output_filename = f"boxplot_triple_k_comparison_r{fixed_r}.png"
