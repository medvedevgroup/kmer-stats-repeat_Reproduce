...
def create_triple_subplot_boxplot(data, thresholds=[32, 630], ylimits=[0.8, 0.05, 1.0], fixed_r=0.01):
    ...
    # Define colors for different methods - removed r_weak and changed r_strong to r_hat
    colors = {"r_hat": "#D00000", "r_Mash": "#D0A000"}
    ...
    def plot_on_axis(ax, data, k_values, panel_index):
        ...
        # Offset for each method's boxplot - now only 2 methods, so adjusted offsets
        offset = [-0.15, 0.15]
        
        # Create boxplots for each method - removed r_weak
    ...
    def set_panel_y_limits(ax, data_subset, max_limit, panel_name):
        if not data_subset.empty:
            # Updated column names - removed r_weak
    ...
    if legend_handles1:
        legend = ax1.legend(
            legend_handles1, 
            [r"$\hat{r}$", "r_Mash", f'True r={fixed_r}'],  # Changed r_strong to \hat{r} in LaTeX
            fontsize=14,
            loc='upper right'
        )
    ...
    plt.suptitle(
        f'Comparison of Two Estimators (r={fixed_r})', 
        fontsize=24
    )
