...
// mpreal calculateMean(const std::vector<mpreal>& numbers) {
//     mpreal sum = 0.0;
...
// mpreal calculateStandardError(const std::vector<mpreal>& numbers) {
//     mpreal mean = calculateMean(numbers);
//     mpreal squaredDiffSum = 0.0;
//     for (const auto& num : numbers) {
//         mpreal diff = num - mean;
//         squaredDiffSum += diff * diff;
//     }
//     mpreal n = mpreal(numbers.size());
...
int main (int argc, char* argv[]){
    ...
    for(int i=0; i<num_replicates; i++){
        ...
        //mean_rhat += r_hat;
        cout<< "#" << i << ": " << r_hat << endl;
        //r_hat_vector.push_back(r_hat);
        MSE += abs(r_hat - r)/r;
        
    }
    // mpreal mean_r_hat = calculateMean(r_hat_vector);
    // mpreal zscore = abs(mean_r_hat - r) / (calculateStandardError(r_hat_vector)) ;
    MSE = MSE / num_replicates;
    //mean_rhat = mean_rhat/num_replicates;
    //mpreal Normalized_RMSE = sqrt(MSE) / r;
    cout << r << "," << k << "," << MSE << endl;
    //cout << r << "," << k << "," << (mean_rhat-r)/r  << endl;
